...
void setup() {
    ...
    client.subscribe("TeamET/room2/neopixel/cmd");
    client.subscribe("TeamET/room2/servo/cmd");
}
...
void loop() {
    ...
    if(currentMillis - lastPublished >= pubInterval) {
        ...
        client.publish("TeamET/room2/sensor/evt/temp", buf);
        sprintf(buf, "%.1f", humidity);
        client.publish("TeamET/room2/sensor/evt/humid", buf);
        sprintf(buf, "%d", light);
        client.publish("TeamET/room2/sensor/evt/light", buf);
    }
}
...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    if(!strcmp(topic, "TeamET/room2/neopixel/cmd")) {
        int i;
    ...
    else if(!strcmp(topic, "TeamET/room2/servo/cmd")) {
        int i;
        for(i = 0; i < (int)length; i++) {
            msgBuffer[i] = payload[i];
        } 
